...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 BBX.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzBBXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzBBXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzBBXReset.connect(boost::bind(NotifyzBBXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzBBXReset.disconnect(boost::bind(NotifyzBBXReset, this));
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid Ballotbox address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid Ballotbox address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from Ballotbox address");
