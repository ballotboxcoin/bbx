...
#include "qt/ballotbox/privacywidget.h"
#include "qt/ballotbox/forms/ui_privacywidget.h"
#include "qt/ballotbox/qtutils.h"
#include "guiutil.h"
#include "qt/ballotbox/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(BallotboxGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zBBX anonymizes your BBX by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zBBX or convert back to BBX"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 BBX ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zBBX");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zBBX");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zBBX");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zBBX");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zBBX");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zBBX");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zBBX");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zBBX");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zBBX Movements"));
    ...
    // Only Convert to BBX enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzBBX->setTitleClassAndText("btn-title-grey", "Total 0 zBBX");
    ui->btnTotalzBBX->setSubTitleClassAndText("text-subtitle", "Show denominations of zBBX owned.");
    ui->btnTotalzBBX->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BBX outputs to mint into zBBX.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zBBX");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzBBX, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zBBX");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of BBX to mint into zBBX"));
    }else{
        btnText = tr("Convert back to BBX");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zBBX to convert back into BBX"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzBBX->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzBBX->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zBBX is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zBBX, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zBBX").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zBBX minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zBBX converted back to BBX"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::onCoinControlClicked(){
    if(ui->pushRight->isChecked()) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any BBX to select."));
        }
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBBX </b>";
    ...
    ui->btnTotalzBBX->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
