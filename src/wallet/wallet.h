...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 BBX at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBBX send
enum ZerocoinSpendStatus {
    ZBBX_SPEND_OKAY = 0,                            // No error
    ZBBX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBBX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBBX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBBX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBBX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBBX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBBX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBBX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBBX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBBX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBBX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBBX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBBX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBBX_SPENT_USED_ZBBX = 14,                      // Coin has already been spend
    ZBBX_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZBBX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZBBXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzBBXTracker> zpivTracker;
    ...
    void setZWallet(CzBBXWallet* zwallet);
    CzBBXWallet* getZWallet();
    ...
    /// Get 10000 BBX output and keys which can be used for the Masternode
    ...
    /** zBBX reset */
    boost::signals2::signal<void()> NotifyzBBXReset;
