// Copyright (c) 2017-2019 The Ballotbox developers
...
CzBBXWallet::CzBBXWallet(std::string strWalletFile)
{
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBBXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBBXSeed_deprecated()) {
                    LogPrintf("%s: Updated zBBX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
...
bool CzBBXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBBXCount(nCountLastUsed);
    else if (!walletdb.ReadZBBXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBBXWallet::Lock()
{
    seedMaster = 0;
}

void CzBBXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBBXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBBX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBBXWallet::LoadMintPoolFromDB()
{
...
void CzBBXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBBXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBBXWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzBBXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBBX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzBBXWallet::SeedToZBBX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBBXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBBXWallet::GenerateDeterministicZBBX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBBXWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBBXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzBBXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
