// Copyright (c) 2018-2020 The Ballotbox developers
...
CzBBXTracker::CzBBXTracker(std::string strWalletFile)
{
...
CzBBXTracker::~CzBBXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBBXTracker::Init()
{
...
bool CzBBXTracker::Archive(CMintMeta& meta)
{
...
bool CzBBXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBBXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBBXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBBXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBBXTracker::GetSerialHashes()
{
...
CAmount CzBBXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBBXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBBXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBBXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBBXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBBXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBBXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBBXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBBXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBBXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBBXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzBBXWallet* zBBXWallet)
{
    bool iszBBXWalletInitialized = (NULL != zBBXWallet);
    CMintMeta meta;
    ...
    if (! iszBBXWalletInitialized)
        zBBXWallet = new CzBBXWallet(strWalletFile);
    meta.isSeedCorrect = zBBXWallet->CheckSeed(dMint);
    if (! iszBBXWalletInitialized)
        delete zBBXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzBBXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBBXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBBXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBBXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBBXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBBXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzBBXWallet* zBBXWallet = new CzBBXWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zBBXWallet);
        }
        delete zBBXWallet;
